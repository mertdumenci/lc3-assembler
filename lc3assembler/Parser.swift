...
func parseHex(string: String) -> UInt16? {
    ...
    let secondIndex = strippedString.index(strippedString.startIndex,
            offsetBy: 1)
...
func parseResolvable(string: String) -> ResolvableOffset {
    ...
    return ResolvableOffset(value: nil, symbol: strippedString)
}
...
func deconsBR(opcode: String)
                -> (opcode: String, conditionCode: ConditionCode)? {
    var firstTwo = opcode
...
enum Instruction {
    case ADD(dataRegister: UInt8,
             sourceRegister1: UInt8,
             sourceRegister2: UInt8)
    case ADD_IMMEDIATE(dataRegister: UInt8,
                       sourceRegister: UInt8,
                       immediateValue: Int8)
    case AND(dataRegister: UInt8,
             sourceRegister1: UInt8,
             sourceRegister2: UInt8)
    case AND_IMMEDIATE(dataRegister: UInt8,
                       sourceRegister: UInt8,
                       immediateValue: Int8)
    case BR(conditionCode: ConditionCode,
            offset: ResolvableOffset)
    case JMP(baseRegister: UInt8)
    case JSR(offset: ResolvableOffset)
    case JSRR(baseRegister: UInt8)
    case LD(dataRegister: UInt8,
            offset: ResolvableOffset)
    case LDI(dataRegister: UInt8,
             offset: ResolvableOffset)
    case LDR(dataRegister: UInt8,
             baseRegister: UInt8,
             offset: ResolvableOffset)
    case LEA(dataRegister: UInt8,
             offset: ResolvableOffset)
    case NOT(dataRegister: UInt8,
             sourceRegister: UInt8)
    case RET
    case RTI
    case ST(sourceRegister: UInt8,
            offset: ResolvableOffset)
    case STI(sourceRegister: UInt8,
             offset: ResolvableOffset)
    case STR(sourceRegister: UInt8,
             baseRegister: UInt8,
             offset: ResolvableOffset)
    case TRAP(trapVector: UInt8)

    static let opcodes =
            ["ADD", "AND", "BR", "BRn", "BRnz", "BRnp", "BRnzp", "BRz",
             "BRzp", "BRp", "JMP", "JSR", "JSRR", "LD", "LDI", "LDR", "LEA",
             "NOT", "RET", "RTI", "ST", "STI", "STR", "TRAP"]
}

protocol Parseable {
    // TODO: Don't use Assembly<T> here
    // extract that part to a separate function
    static func parse(line: LabeledLine, address: UInt16)
                    -> Assembly<BinaryRepresentable>?
}
...
extension AssemblerDirective: Parseable {
    static func parse(line: LabeledLine, address: UInt16)
        ...
        guard let opcode = tokens.first,
              AssemblerDirective.opcodes.contains(opcode) else {
            return nil
        }
        ...
        if let directive = directive {
            return Assembly(assembly: directive,
                            label: line.label,
                            address: address)
        }
...
extension Instruction: Parseable {
    static func parse(line: LabeledLine, address: UInt16)
        ...
        guard let opcode = tokens.first,
              Instruction.opcodes.contains(opcode) else {
            return nil
        }
        ...
        case "AND":
            if let dataRegister = parseRegister(string: rest[1]),
               ...
               let sourceRegister2 = parseRegister(string: rest[3]) {
                ...
                // Special case for BR, as the branch conditions
                // are inside the opcode i.e. BRn, BRnzp, BRz, BR, et
        case _ where deconsBR(opcode: opcode) != nil:
            // TODO: I don't like the 2x calls to deconsBR
        ...
        case "NOT":
            if let dataRegister = parseRegister(string: rest[1]),
               let sourceRegister = parseRegister(string: rest[2]) {
                instruction = .NOT(dataRegister: dataRegister,
                                   sourceRegister: sourceRegister)
            }
        ...
        if let instruction = instruction {
            return Assembly(assembly: instruction,
                            label: line.label,
                            address: address)
        }
...
func parse(assembly: String) -> [Assembly<BinaryRepresentable>]? {
    ...
    for line in lines {
        if let directive = AssemblerDirective.parse(line: stripLabel(line),
                                                    address: 0x0),
           case .ORIG(let address) = (directive.assembly as! AssemblerDirective) {
            // TODO: Find a way that doesn't need a force cast
